...
parser.add_argument(
    ...
    type=str,
    default="causal_lm",
    choices=["causal_lm", "embedding"],
    help="model loading mode; use embedding for AutoModel-based embedding models",
)
...
parser.add_argument(
    ...
    type=str,
    default="pileval",
    help="calibration data source passed to get_calib_dataset",
)
parser.add_argument(
    "--calib_n_samples",
    type=int,
...
parser.add_argument(
    ...
    type=int,
    default=512,
    help="calibration sequence length for AWQ search",
)
parser.add_argument(
    "--vila-15",
    action="store_true",
...
vila_10_quant_mode = (
    ...
    and ("llava" in args.model_path.lower() or "vila" in args.model_path.lower())
...
def build_model_and_enc(model_path, dtype):
    ...
    if vila_10_quant_mode:
        ...
        if enc.pad_token_id is None and enc.eos_token_id is not None:
            enc.pad_token = enc.eos_token

    model_cls = AutoModel if args.model_type == "embedding" else AutoModelForCausalLM
    ...
    if args.tasks is not None:
        ...
        if args.tasks == "wikitext":
            ...
            try:
                ...
                raise ImportError(
                    "lm_eval is required for non-wikitext evaluation tasks. "
                    "Install lm_eval or omit --tasks for quantization-only runs."
                ) from exc
