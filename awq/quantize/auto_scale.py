...
QWEN_DECODER_LAYERS = tuple(
    layer for layer in (Qwen2DecoderLayer, Qwen3DecoderLayer) if layer is not None
)
QWEN_RMS_NORMS = tuple(norm for norm in (Qwen2RMSNorm, Qwen3RMSNorm) if norm is not None)


def _is_qwen_decoder_layer(module):
    class_name = module.__class__.__name__.lower()
    return isinstance(module, QWEN_DECODER_LAYERS) or (
        "qwen" in class_name and "decoderlayer" in class_name
    )


def _is_supported_rms_norm(module):
    class_name = module.__class__.__name__.lower()
    return isinstance(module, (LlamaRMSNorm,) + QWEN_RMS_NORMS) or (
        "qwen" in class_name and "rmsnorm" in class_name
    )
...
def auto_scale_block(module, module_kwargs, w_bit, q_config, input_feat):
    ...
    elif isinstance(module, LlamaDecoderLayer) or _is_qwen_decoder_layer(module):
        # attention input
        scales_list.append(
            _auto_get_scale(
...
def apply_scale(module, scales_list, input_feat_dict=None):
    for prev_op_name, layer_names, scales in scales_list:
        ...
        elif isinstance(prev_op, nn.LayerNorm) or _is_supported_rms_norm(prev_op):
