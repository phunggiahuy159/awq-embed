...
def get_blocks(model):
    if (
        _is_qwen_or_llama(model)
        and hasattr(model, "model")
        and hasattr(model.model, "layers")
    ):
        layers = model.model.layers
    elif _is_qwen_or_llama(model) and hasattr(model, "layers"):
        layers = model.layers
...
def _move_module_attr(module, attr_name, device):
    if hasattr(module, attr_name):
        setattr(module, attr_name, getattr(module, attr_name).to(device))
...
def move_embed(model, device):
    if (
        ...
        _move_module_attr(model.model, "rotary_emb", device)
    elif _is_qwen_or_llama(model) and hasattr(model, "embed_tokens"):
        model.embed_tokens = model.embed_tokens.to(device)
        _move_module_attr(model, "rotary_emb", device)
    elif model.__class__.__name__ == "InternVL3":
